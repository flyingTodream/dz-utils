...
console.log();
console.log(
  chalk.yellowBright(`⚠️ You should have run ${chalk.bold('`npm run release`')} before running this script!`)
);
console.log();
...
const fileVersions = ['package.json', 'package-lock.json', 'dist/dz-utils.js'].map((filePath) => ({
  file: filePath,
  version: require('../' + filePath).version,
}));

['lib/index.js', 'src/index.ts'].forEach((filePath) => {
...
const npmignorePatterns = npmignore.split(/\r?\n/).filter((item) => item && !item.startsWith('#'));
...
if (untrackedFiles.length) {
  ...
  if (maybeUnexpectedFiles.length) {
    console.error(
      chalk.red(
        `❌ Found ${maybeUnexpectedFiles.length} file(s) that are neither tracked by git nor ignored by .npmignore! Please double-check before publishing them to npm.`
      )
    );
