...
const outputOption = {
  format: 'umd',
  file: path.resolve(__dirname, '../dist/dz-script.js'),
  sourcemap: false,
  name: 'dzUtils',
};
...
if (processs.argv.includes('--watch')) {
  ...
  watcher.on('event', (event) => {
    switch (event.code) {
      ...
      //     break;
      case 'BUNDLE_START':
        console.log(chalk.gray(current()), chalk.blue('File changed. Begin to bundle'));
        break;
      case 'BUNDLE_END':
        console.log(chalk.gray(current()), chalk.green('Finished bundle'));
        break;
      case 'ERROR':
        console.log(chalk.gray(current()), chalk.red(event.error));
        break;
    }
  });
} else {
  ...
  rollup
    ...
    .then((bundle) => {
      bundle.write(outputOption).then(() => {
        // Minified
